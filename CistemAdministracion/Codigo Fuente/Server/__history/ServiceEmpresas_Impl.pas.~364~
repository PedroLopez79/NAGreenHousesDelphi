...
type
  ...
  protected
    ...
    function obtendatosfotosnombresempleados(const Nombres: AnsiString): AnsiString;
    ...
    function obtenIDEmpleado: Integer;
    function obtenRostros10(const idemp: AnsiString): AnsiString;
    function obtenListaSyncronizado(const listado1: AnsiString; const listado2: AnsiString): AnsiString;
  end;
...
function TServiceEmpresas.obtendatosfotosnombresempleados(
  const Nombres: AnsiString): AnsiString;
var
  ds: IDADataSet;
  res: String;
begin
  ds:=Schema.NewDataset(Connection, 'spObtenFotosNombresEmpleados');
  ds.ParamByName('NOMBREEMPLEADO').AsString:= Nombres;
  ds.Open;
...
function TServiceEmpresas.obtenIDEmpleado: Integer;
var
  ds: IDADataSet;
begin
  ds:=Schema.NewDataset(Connection, 'spObtenIDEmpleado');
  ds.Open;
  Result:=ds.FieldByName('IDEmpleado').AsInteger;
  ds.Close;
end;

function TServiceEmpresas.obtenListaSyncronizado(
  const listado1: AnsiString; const listado2: AnsiString): AnsiString;
var
  ds1: IDADataSet;
  ds2: IDADataSet;
  ds3: IDADataSet;
begin
  result:='';
  ds1:=Schema.NewDataset(Connection, 'spObtenListadoSyncronizar');
  ds1.Close();

  ds1.ParamByName('listado1').AsString:= listado1;
  ds1.ParamByName('listado2').AsString:= listado2;
  ds1.Open;

  if not ds1.IsEmpty then
  begin
    result:= ds1.FieldByName('XML').AsString;
  end;
  ds1.Close;
...
function TServiceEmpresas.obtenRostros10(const idemp: AnsiString): AnsiString;
var
  ds: IDADataSet;
  res: String;
begin
  ds:=Schema.NewDataset(Connection, 'spObtenRostros10');
  ds.ParamByName('idemp').AsString:= idemp;
  ds.Open;
